    ...
    public class FuncaoAutomacao
    {
        ...
        public void DoOnLoop(Boolean canLog, String FileName)
        {
            ...
            else
            {
                if (DateTime.Now.Subtract(dataUltimaConexao).TotalSeconds > delay)
                {
                    ...
                    CriaLog(996, "Tentativa " + nTentativas.ToString() + ". A conexão com a SEFAZ esta indisponivel. Nova consulta em " + delay.ToString() + " segundos.");
                }
            }
                ...
                if (ListaDePedidosCartaDeCorrecao.Count > 0)
                {
                    foreach (String origem in ListaDePedidosCartaDeCorrecao)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                throw new Exception("Não foi possível assinar o arquivo : ERRO" + CodigoDoResultado.ToString());
                            }
        ...
        public void CriaLog(int codigo, string origem, Exception ex, string chaveNFe = "")
        {
            ...
            if (_enviarErros)
            {
                try
                {
                    //TODO : inicializar uma thread que irá fazer o envio do erro para nossa base de dados
        ...
        public Boolean ConsultaStatus()
        {
            ...
            catch (Exception ex)
            {
                ...
                CriaLog(999, "ConsultaStatus SEFAZ-" + NFeUtils.ObterDescricao(oParam.UF) + " não respondeu no timeout configurado. ",
                    ex);
                retorno = false;
            }
            finally
